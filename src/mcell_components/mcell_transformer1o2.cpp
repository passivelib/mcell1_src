...
int Transformer1o2::threadNumber(Command *cm)
{
...
int Transformer1o2::drc(double dp, double ds, double wp, double ws, double s, double sp, double lp, double wtp, double wts, int geometry, string topM)
{
  ...
  if (wp < topMId.minW or ws < topMId.minW or ws < bottomMId.minW)
  {
  ...
  double ds2 = ds - 4 * ws - 2 * s; // inner secondary diameter

  if (ds2 < 2 * bottomMId.minS)
  {
    throw SoftError("Warning: inner diameter too small!");
  }
  ...
  if (sp < b + 2 * ws + 2 * topMId.minS and 0.5 * ds > 0.5 * dp - wp - topMId.minS)
  {
  ...
  if (wts > 0 and sp < wts + 2 * bottomMId.minS)
  {
    throw SoftError("Warning: pin space too short!");
  }
...
int Transformer1o2::genCell(Cell &result, double dp, double ds, double wp, double ws, double s, double sp, double lp, double wtp, double wts, int geometry, string topM, bool fasterCap)
{
  // ###############################################################################################
  // # transformer1o2
  // # 		: generate cell
  ...
  double e = (ws + s);
  // do not round it since ws and s are rounded
  // if it is rounded bridge45 can be shifted
  ...
  this->l->crop(path1Arr, path1, p1c, p2c, bottomM);
  this->l->crop(path2Arr, path2, p1c, p2c, bottomM);
  ...
  // ###############################################################################################
  // # secondary inner turn
  // ###############################################################################################

  this->l->drawSegment(path0, ds - 2 * (ws + s), ws, 0, geometry, bottomM);
  this->l->drawSegment(path1, ds - 2 * (ws + s), ws, 1, geometry, bottomM);
  this->l->drawSegment(path2, ds - 2 * (ws + s), ws, 2, geometry, bottomM);
  this->l->drawSegment(path3, ds - 2 * (ws + s), ws, 3, geometry, bottomM);

  // fasterCap
  if (fasterCap)
  {
    this->l->crop(path1Arr, path0, p1c_ws, p2c_ws, bottomM);
    this->l->crop(path2Arr, path3, p1c_ws, p2c_ws, bottomM);
  ...
  // #         * w *
  // #####################################################################################################
  p1 = Vec2{-0.5 * ds + e, 0.5 * (b)};
  p2 = Vec2{-0.5 * ds + e + ws, 0.5 * (b)};
  Vec2 p3 = Vec2{-0.5 * ds, -0.5 * (b)};
  Vec2 p4 = Vec2{-0.5 * ds + ws, -0.5 * (b)};

  this->l->bridge45Vias2(path1Arr, p1, p2, p3, p4, bottomM, topM);
...
int Transformer1o2::process(Command *cm)
{
  ...
  if (cm->generateSpiceModel == 1)
  {
    ...
    {
      cm->cellName = "plTr1o2Oct";
    }
    else if (cm->geometry == 1) // rectangular
    {
      cm->cellName = "plTr1o2Rect";
    }
  ...
  auto threadFunc = [this, &syncronization_flag, &counter, &runEmxShFile, &maestroFile, cm, sweepFlag](double d, double w, double ws, double s, double sh)
  {
    try
    {
      ...
      try
      {
        ...
        // calc(double& L, double d, double w, double ws, double s, double ss, double sp, double lp, double wtp, double wts, int geometry, string topM)
        ...
        if (cm->generateSpiceModel == 1) // model generator
        {
          ...
          std::cout << endl
                    << std::left;
          std::cout << std::setw(75) << "transformer1o2: d=" + dStr + " w=" + wStr + " ws=" + wsStr + " s=" + sStr + " sh=" + ssStr;
          counter++;
...
int Transformer1o2::FastHenryGenCell(FastHenry *fh, double dp, double ds, double wp, double ws, double s, double sp, double lp, double wtp, double wts, int geometry, string topM, bool fasterCap)
{
  // ###############################################################################################
  // # transformer1o2
  // # 		: generate cell
  ...
  fh->l.crop(path1Arr, path1, p1c, p2c, bottomM);
  fh->l.crop(path2Arr, path2, p1c, p2c, bottomM);
  ...
  fh->l.drawSegment(path0, ds - 2 * (ws + s), ws, 0, geometry, bottomM);
  fh->l.drawSegment(path1, ds - 2 * (ws + s), ws, 1, geometry, bottomM);
  fh->l.drawSegment(path2, ds - 2 * (ws + s), ws, 2, geometry, bottomM);
  fh->l.drawSegment(path3, ds - 2 * (ws + s), ws, 3, geometry, bottomM);
  ...
  if (fasterCap)
  {
    ...
    // ports
    portS1n.x = 0.5 * (ds - 2 * (ws + s) - ws);
    portS1n.y = -0.5 * ws;

    portS2n.x = 0.5 * (ds - 2 * (ws + s) - ws);
    portS2n.y = 0.5 * ws;
  }
  else
  {
  ...
  // #         * w *
  // #####################################################################################################
  p1 = Point2(-0.5 * ds + e, 0.5 * (b));
  p2 = Point2(-0.5 * ds + e + ws, 0.5 * (b));
  Point2 p3 = Point2(-0.5 * ds, -0.5 * (b));
  Point2 p4 = Point2(-0.5 * ds + ws, -0.5 * (b));

  fh->l.bridge45Vias2(path1Arr, p1, p2, p3, p4, bottomM, topM);
  ...
  if (wts > 0)
  {
    ...
    p2 = Point2(0.5 * ds - 2 * ws - s, -0.5 * wts);
...
int Transformer1o2::FastHenryProcess(Command *cm)
{
  // ###############################################################################################
...
int Transformer1o2::FastHenry2Process(Command *cm)
{
  // ###############################################################################################
  // # FastHenryTransformer
  // #		: process command line options

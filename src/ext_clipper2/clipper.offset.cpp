...
namespace Clipper2Lib
{
	...
	ClipperOffset::Group::Group(const Paths64 &_paths, JoinType _join_type, EndType _end_type) : paths_in(_paths), join_type(_join_type), end_type(_end_type)
	{
		bool is_joined =
	...
	void ClipperOffset::DoRound(const Path64 &path, size_t j, size_t k, double angle)
	{
		if (deltaCallback64_)
		{
			...
			double arcTol = (arc_tolerance_ > floating_point_tolerance ? std::min(abs_delta, arc_tolerance_) : std::log10(2 + abs_delta) * default_arc_tolerance);
	...
	void ClipperOffset::DoGroupOffset(Group &group)
	{
		...
		for (; path_in_it != group.paths_in.cend(); ++path_in_it, ++is_hole_it, ++area_it)
		{
			...
			if (is_shrinking && (std::fabs(*area_it) < min_area))
				continue;
			...
			if ((pathLen == 2) && (group.end_type == EndType::Joined))
				end_type_ = (group.join_type == JoinType::Round) ? EndType::Round : EndType::Square;

			BuildNormals(*path_in_it);
			if (end_type_ == EndType::Polygon)
				OffsetPolygon(group, *path_in_it, is_shrinking, *area_it);
